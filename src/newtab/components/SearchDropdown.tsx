...
function getProviderIcon(provider: Provider) {
  return (
    provider.iconUrl ||
    getFaviconFromPattern(provider.urlPattern) ||
    "/assets/new_tab_search/mitria.svg"
  );
}
...
export function SearchDropdown({
  ...
  const enabledProviderIds = useProviderStore(
    (state) => state.enabledProviderIds
  );
      ...
      {providers.map((provider, index) => {
        ...
        return (
          ...
          >
            ...
            />
            <span className="flex-shrink-0 w-32 text-sm text-foreground">
              {provider.name}
            </span>
