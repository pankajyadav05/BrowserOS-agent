...
function getFaviconFromPattern(urlPattern: string) {
  try {
    const sample = urlPattern.includes("%s")
      ? urlPattern.replace("%s", "search")
      : urlPattern;
...
function getProviderIcon(provider: Provider) {
  ...
  return "/assets/new_tab_search/mitria.svg";
}
...
export function ProviderSettings() {
  const enabledProviders = useProviderStore((state) =>
    state.getEnabledProviders()
  );
  const disabledProviders = useProviderStore((state) =>
    state.getDisabledProviders()
  );
  ...
  const reorderEnabledProviders = useProviderStore(
    (state) => state.reorderEnabledProviders
  );
  const reorderDisabledProviders = useProviderStore(
    (state) => state.reorderDisabledProviders
  );
  const addCustomProvider = useProviderStore(
    (state) => state.addCustomProvider
  );
  const removeCustomProvider = useProviderStore(
    (state) => state.removeCustomProvider
  );
  const hasLegacySynced = useProviderStore((state) => state.hasLegacySynced);
  const importLegacyProviderSettings = useProviderStore(
    (state) => state.importLegacyProviderSettings
  );

  const [isFormOpen, setIsFormOpen] = useState(false);
  const [formState, setFormState] = useState<AddProviderFormState>({
    name: "",
    urlPattern: "",
  });
  const [formError, setFormError] = useState<string | null>(null);
  const [draggedProvider, setDraggedProvider] = useState<Provider | null>(null);
  const [dragSource, setDragSource] = useState<"enabled" | "disabled" | null>(
    null
  );
    ...
    if (dragSource === targetList) {
      if (targetList === "enabled") {
        reorderEnabledProviders(
          enabledProviders.findIndex(
            (provider) => provider.id === draggedProvider.id
          ),
          targetIndex
        );
      } else {
        reorderDisabledProviders(
          disabledProviders.findIndex(
            (provider) => provider.id === draggedProvider.id
          ),
          targetIndex
        );
  ...
  const renderProviderRow = (
    ...
    return (
      <div
        ...
        onDragLeave={() =>
          setDragOver((current) =>
            current?.index === index && current.list === list ? null : current
          )
        }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Reorder providers, disable ones you do not use, or add your own
            search shortcuts.
          </p>
        </div>
        <button
      ...
      {isFormOpen && (
        ...
        >
          ...
          <p className="text-[11px] text-muted-foreground">
            Paste the provider home or search URL. We will grab the favicon
            automatically.
          </p>
        ...
        >
          {enabledProviders.length === 0 ? (
            ...
            enabledProviders.map((provider, index) =>
              renderProviderRow(provider, index, "enabled")
            )
          )}
        </div>
      </div>
        ...
        >
          {disabledProviders.length === 0 ? (
            ...
            disabledProviders.map((provider, index) =>
              renderProviderRow(provider, index, "disabled")
            )
          )}
        </div>
      </div>
